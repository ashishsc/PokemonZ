...
namespace TestGame
{
    public class FlamethrowerEngine : GameObject
    {
        ...
        public FlamethrowerEngine(List<Texture2D> textures, Vector2 location) : base(textures[0], location)
        {
            direction = 1;
        ...
        private FlamethrowerParticle GenerateNewParticle()
        {
            ...
            Vector2 velocity = new Vector2(7f * direction, (float)(random.NextDouble() * 2 - 1));
            float angle = 3.14f;
        ...
        new public Rectangle BoundingBox
        {
            get
            {
                return new Rectangle((int)EmitterLocation.X, (int)EmitterLocation.Y - 25, 200, 50);
            }
        }
